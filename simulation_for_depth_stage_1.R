...
##source("visualization_based_paramerization.R")
...
sparse_simulation <- function(simul_set, param_sim0_PD, n, nbvar, standard_grid, 
                              outlier_set, 
  ...
  ###################
  for (depth_count in 1:length(notes)) {
    cat(depth_count, "\n")
    if (depth_count <= 6) {
      ...
      true_data <- data_structure_transform(nbvar, standard_grid, 
                                            n, param = param_sim0_PD)
      sim0_mfd <- simulation_mfhd(true_data, standard_grid, n, nbvar)
      sparse_data <- data_structure_transform(nbvar, standard_grid, 
                                              n, param = param_sim1_PD)
      start_time <- Sys.time()
      ...
      median_measure_sum[depth_count] <- measures[[1]]
      central_measure_sum[depth_count] <- measures[[2]]
      outliers_measure_sum[depth_count] <- measures[[3]]
      rank_measure_sum[depth_count] <- measures[[4]]
      running_time_sum[depth_count] <- measures[[5]]
      
  }  
  
  names(median_measure_sum) <- notes
  names(central_measure_sum) <- notes
  names(outliers_measure_sum) <- notes
  names(rank_measure_sum) <- notes
  names(running_time_sum) <- notes
  return (list(median_measure = median_measure_sum, 
             central_measure = central_measure_sum, 
             outlier_measure = outliers_measure_sum,
             rank_measure = rank_measure_sum,
             running_time_measure = running_time_sum))
}
