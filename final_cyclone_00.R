directory <- "~/Dropbox/Mac/Desktop/KAUST/project/Clustering/clustering_code/code"
setwd(directory)
packages = c("ggmap",
             "ggplot2", "readr","dplyr")
...
#### west Pacific
clustl <- c(1, 8)
westpacific <- unlist(lapply(subsetlist(clust_global$clust, clustl), function(l)
  {l$element}))
save(westpacific, file = "pacific_index.RData")
...
save(neighbours_westpacific, file = "neighbours_westpacific.RData")
...
load("pacific_index.RData")
load("neighbours_westpacific.RData")
load("clust_westpacific.RData")
...
ggplot(aes(x = long, y = lat), data = worldmap) + 
  ...
  labs(title = expression(bold("(a) Northwest Pacific Cyclone Tracks")), x = "Longitude",y = "Latitude") +
  theme(plot.title = element_text(hjust = 0.5, size = 15), 
        axis.text = element_text(size = 14), 
        axis.title = element_text(size = 13),
        plot.margin = margin(1, 1.2, 0.5, 1)) + 
  scale_colour_gradient(low = "red", high = "yellow", na.value = NA,
                        breaks = seq(0, 1, by = 0.2), labels = seq(0, 100, by = 20),
                        limits = c(0, 1))
dev.off()
